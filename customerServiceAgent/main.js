...
const WORKFLOW_ID = '7617784508758605851'; // 固定工作流ID
 ...
 * @param {object} extractParams - extract_params对象，用于trade等action
 */
function renderActionBlock(action, messageElement, extractParams = {}) {
    ...
    switch(actionType) {
        ...
        case 'trade':
            // trade类型：交易确认按钮
            const tradeContent = content || '执行交易';
            let tradeDetails = '';
            if (extractParams && Object.keys(extractParams).length > 0) {
                tradeDetails = Object.entries(extractParams).map(([key, value]) => `${key}: ${value}`).join(', ');
                tradeDetails = ` (${tradeDetails})`;
            }
            actionBlock.html(`
                <button class="action-btn action-btn-primary trade-confirm">${tradeContent}${tradeDetails}</button>
                <button class="action-btn action-btn-secondary trade-cancel">取消</button>
            `);
            // 绑定点击事件
            actionBlock.find('.trade-confirm').on('click', function() {
                const btn = $(this);
                btn.prop('disabled', true).text('交易执行中');
                log(`【${tradeContent}】交易执行中...`);
                // 这里可以添加实际的交易API调用逻辑
            });
            actionBlock.find('.trade-cancel').on('click', function() {
                actionBlock.remove();
            });
            break;
        case 'human':
        ...
        case 'html':
            ...
            break;
        case 'open':
            // open类型：通栏主按钮，打开平台内部页面
            // 检查path是否为空，为空则不渲染
            if (!path || path.trim() === '') {
                return;
            }
            // 假设path是相对路径，添加平台基础URL
            const BASE_URL = 'https://platform.example.com'; // 根据实际平台调整
            let openHref = path.startsWith('/') ? BASE_URL + path : BASE_URL + '/' + path;
            const openContent = content || '打开页面';
            actionBlock.html(`<a class="action-btn action-btn-primary action-btn-full open-link" href="${openHref}" target="_blank" rel="noopener noreferrer">${openContent}</a>`);
            break;
        default:
            // 未知类型不渲染
            return;
 ...
 * @param {object} extractParams - extract_params对象，用于trade等action
 */
function addBotMessage(content, isStreaming = false, action = null, extractParams = {}) {
    ...
    if (!isStreaming || !currentBotMessageElement) {
        ...
        if (action) {
            ...
            renderActionBlock(action, bubbleContainer, extractParams); // 将action方块添加到气泡容器内
        }
    } else {
        // 流式更新现有消息
    ...
    try {
        ...
        if (result?.state != 1) {
            ...
            if (resultArr.length === 1) {
                ...
                /* ===================== 敏感词过滤结束 ===================== */
                addBotMessage(filteredContent, false, action, item.extract_params);
                ...
                for (let i = 0; i < resultArr.length; i++) {
                    ...
                    addBotMessage(messageContent, false, action, item.extract_params);
